...
int mainn(int argc, char* argv[])
{
    ...
    char *message= static_cast<char *>(malloc(maxchars * sizeof(char)));  //allocate memory for our message string
...
int texteditor(char *text, int maxchars, const int startrow, const int startcol,int maxrows,const int maxcols,const int displayrows,const int returnhandler,const char *permitted, bool ins, const bool allowcr)
{
    ...
    if ((display = static_cast<char **>(malloc(maxrows * sizeof(char *)))) == NULL) /*assign memory equal to number of maxrows to display*/
    ...
    for(ky = 0; ky < maxrows; ky++)		//now traverse all rows in display and allocated and assign memory equal to maxcols to each row of display
        if ((display[ky] = static_cast<char *>(malloc((maxcols + 1) * sizeof(char)))) == NULL)
            malloc_error();

    if ((original= static_cast<char *>(malloc(maxchars * sizeof(char)))) == NULL)	//allocated memory to original
        malloc_error();
    strcpy(original,text);//copy text taken from file to original variable

    if ((savetext= static_cast<char *>(malloc(maxchars * sizeof(char)))) == NULL)// allocate memory to savetext
        malloc_error();
...
int search_string(char *string, char* data)
{
    ...
    {
        s_a='y';
        string = static_cast<char *>(malloc(500 * sizeof(char)));
...
int replace_string(char* data)
{
    ...
    replace_with= static_cast<char *>(malloc(500 * sizeof(char)));
    move(32, 0);deleteln();
